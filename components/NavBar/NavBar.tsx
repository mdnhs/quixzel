...
const NavBar = (props: NavBarProps) => {
  return (
    <div className=" bg-transparent text-xl text-primary">
      <div className="w-full h-40 container flex items-center">
        <div className="flex gap-2 basis-1/3 items-center justify-center">
          {leftMenus.map((leftItems, leftIdx) => {
            return (
              <p className="h-10 w-28 cursor-pointer" key={leftIdx + "_" + leftItems.id}>
                {leftItems.name}
              </p>
            );
          })}
        </div>
        <div className="basis-1/3 flex justify-center cursor-pointer">Logo</div>
        <div className="flex gap-2 basis-1/3 justify-end">
          {" "}
          <div className="flex gap-2 basis-1/3 items-center justify-center">
            {rightMenus.map((rightItems, rightIdx) => {
              return (
                <p className="h-10 w-28 cursor-pointer" key={rightIdx + "_" + rightItems.id}>
                  {rightItems.name}
                </p>
              );
            })}
